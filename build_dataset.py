...
def check_log_entry_new(log_entries, compiled_patterns):
...
def process_single_log_file(file_path, compiled_patterns):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        for log in log_entries:
            ...
            matches_list = check_log_entry_new(log.get('stdout_lines', []), compiled_patterns)
            checked_logs +=1
...
def process_log_files(directory_path, compiled_patterns):
    ...
    for filename in filter(lambda f: f.endswith('.json'), os.listdir(directory_path)):
        ...
        summary, task_matches, dataset, checked_logs = process_single_log_file(file_path, compiled_patterns)
